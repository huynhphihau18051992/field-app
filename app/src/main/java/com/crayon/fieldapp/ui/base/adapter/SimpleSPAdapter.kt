...
class SimpleSPAdapter(val context: Context, var items: ArrayList<SimpleModel>) : BaseAdapter() {
    ...
    fun addItems(list: ArrayList<SimpleModel>) {
        items.clear()
        items.addAll(list)
    ...
    fun getPositionByName(mName: String): Int {
        return items.indexOfFirst { it.name.equals(mName) }
    }
}