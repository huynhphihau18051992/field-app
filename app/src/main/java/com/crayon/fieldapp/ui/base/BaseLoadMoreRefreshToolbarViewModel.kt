...
import com.crayon.fieldapp.ui.widgets.EndlessRecyclerOnScrollListener
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.withContext

abstract class BaseLoadMoreRefreshToolbarViewModel<Item>() : BaseViewModel() {

    // id
    val id = MutableLiveData<String>().apply { value = "-1" }

    // title
    val title = MutableLiveData<String>().apply { value = "-1" }
     ...
     * first load
     */
    fun firstLoad(mId: String, mGroupName: String) {
        this.id.value = mId
        this.title.value = mGroupName
        if (isFirst()) {
            showLoading()
